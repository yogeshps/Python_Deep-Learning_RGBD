...
def splitter(classname, picname, size):
    ...
    directory_pre = "/home/astar/Desktop/DL_Stuff/Small_Images_Joy/compiled/"
    directory_train = "/home/astar/Desktop/DL_Stuff/DL_Final/training_set/"
    directory_test = "/home/astar/Desktop/DL_Stuff/DL_Final/test_set/"
    ...
    for i in range(1,size,6):
        ...
        if (i%5==0):
            img_path_save = directory_test + classname + "/" + picname + "_" + str(j) + img_type
            img.save(img_path_save)
            img.save(img_path_save, optimize=True)
            j += 1

        else:
            img_path_save = directory_train + classname + "/" + picname + "_" + str(k) + img_type
            img.save(img_path_save)
            img.save(img_path_save, optimize=True)
            k += 1


splitter("FORWARD", "forward", 4237)
"""splitter("LEFT", "left", 521)
splitter("RIGHT", "right", 724)
splitter("STOP", "stop", 421)"""
